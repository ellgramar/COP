...
def threading_function(file_lock, screen_lock, first_name):
    screen_lock.acquire()
    user_string = get_user_input(first_name)
    screen_lock.release()
    # file_lock
    file_lock.acquire()
    fp = open("mutexrules.txt", "a",1)
...
for i in range(1,length):
    t = Thread(target=threading_function, args=(file_lock, screen_lock, (sys.argv[i])))
    t.start()